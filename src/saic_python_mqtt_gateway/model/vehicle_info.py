...
from saic_python_mqtt_gateway.utils import decode_as_utf8
...
class VehicleInfo:
    def __init__(
        ...
        self.brand: Final[str] = self.normalize_str(vin_info.brandName)
        self.model: Final[str] = self.normalize_str(vin_info.modelName)
        self.model_year: Final[str] = self.normalize_str(vin_info.modelYear)
        self.series: Final[str] = self.normalize_str(vin_info.series)
        self.color: Final[str] = self.normalize_str(vin_info.colorName)
    ...
    def normalize_str(value: str | None) -> str:
        return decode_as_utf8(value, "").strip().upper()
