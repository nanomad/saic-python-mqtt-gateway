...
from saic_python_mqtt_gateway.exceptions import MqttGatewayException
from saic_python_mqtt_gateway.integrations import IntegrationException
from saic_python_mqtt_gateway.integrations.abrp.api import AbrpApi
from saic_python_mqtt_gateway.integrations.home_assistant.discovery import (
    HomeAssistantDiscovery,
)
from saic_python_mqtt_gateway.integrations.osmand.api import OsmAndApi
from saic_python_mqtt_gateway.model import RefreshMode
from saic_python_mqtt_gateway.saic_api_listener import (
...
from saic_python_mqtt_gateway.status_publisher.message import MessagePublisher
from saic_python_mqtt_gateway.status_publisher.vehicle.vehicle_status_resp import (
    VehicleStatusRespPublisher,
)
from saic_python_mqtt_gateway.status_publisher.vehicle_info import VehicleInfoPublisher
...
if TYPE_CHECKING:
    ...
    from saic_python_mqtt_gateway.integrations.openwb.charging_station import (
        ChargingStation,
    )
    from saic_python_mqtt_gateway.model import VehicleInfo
    from saic_python_mqtt_gateway.publisher.core import Publisher
    from saic_python_mqtt_gateway.status_publisher.charge.chrg_mgmt_data_resp import (
...
class VehicleHandler:
    def __init__(
        ...
        charging_station = self.__get_charging_station(vin_info.vin)
        self.vehicle_state: Final[VehicleState] = VehicleState(
            publisher,
            scheduler=scheduler,
            account_prefix=self.vehicle_prefix,
            vin_info=self.vin_info,
            charging_station=charging_station,
            charge_polling_min_percent=self.configuration.charge_dynamic_polling_min_percentage,
        )
        self.__vehicle_info_publisher: Final[VehicleInfoPublisher] = (
            VehicleInfoPublisher(self.vin_info, self.publisher, self.vehicle_prefix)
        )
        self.__vehicle_response_publisher: Final[VehicleStatusRespPublisher] = (
            VehicleStatusRespPublisher(vin_info, publisher, self.vehicle_prefix)
        )
        self.__charge_response_publisher: Final[ChrgMgmtDataRespPublisher] = (
            ChrgMgmtDataRespPublisher(vin_info, publisher, self.vehicle_prefix)
        )
        self.__message_publisher: Final[MessagePublisher] = MessagePublisher(
            vin_info, publisher, self.vehicle_prefix
        )
        self.__ha_discovery = self.__setup_ha_discovery(
            self.publisher, self.vehicle_prefix, vin_info, config
        )
    ...
    async def __polling(self) -> None:
        ...
        charge_status_processing_result: None | ChrgMgmtDataRespProcessingResult = None
        charge_status: None | ChrgMgmtDataResp = None
    ...
    def publish_ha_discovery_messages(self, *, force: bool = False) -> None:
        if self.__ha_discovery is not None and self.vehicle_state.is_complete():
            LOG.info(
    ...
    async def update_vehicle_status(
        ...
        vehicle_status = await self.saic_api.get_vehicle_status(self.vin_info.vin)
        processing_result = self.__vehicle_response_publisher.on_vehicle_status_resp(
            vehicle_status
        )
        self.vehicle_state.handle_vehicle_status(processing_result)
        return vehicle_status, processing_result
    ...
    async def update_charge_status(
        ...
        processing_result = self.__charge_response_publisher.on_chrg_mgmt_data_resp(
            charge_mgmt_data
        )
        self.vehicle_state.handle_charge_status(processing_result)
        return charge_mgmt_data, processing_result
    ...
    def __setup_ha_discovery(
        ...
        if self.configuration.ha_discovery_enabled:
            return HomeAssistantDiscovery(publisher, vehicle_prefix, vin_info, config)
        return None

    def __get_charging_station(self, vin: str) -> ChargingStation | None:
