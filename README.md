...
| /refresh/mode/set                        | periodic/off/force/charging_detection                | The gateway queries the vehicle and charge status periodically after a vehicle start event has happened (default value: periodic). The periodic refresh can be switched off (value: off). A refresh can also be forced (value: force). The charging_detection mode performs a single refresh and resets the shutdown grace period, causing the gateway to poll frequently for ~10 minutes to detect charger connection. |
